    ...
    <div className="flex bg-slate-50 min-h-screen">
      ...
      <main className="flex-grow p-8 md:p-12 max-w-5xl mx-auto">
        <header className="mb-12">
          ...
          <h1 className="text-4xl font-bold tracking-tight font-heading">Create New Offering</h1>
          <p className="text-slate-500 mt-2">Reach thousands of travelers by publishing your premium services.</p>
        </header>
            ...
            <Card className="p-8 border-none shadow-sm space-y-6 rounded-[2rem]">
              <div className="grid md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  <select name="city_id" value={formData.city_id} onChange={handleChange} className="w-full h-12 rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 font-medium text-slate-700 cursor-pointer outline-none">
              ...
              <div className="space-y-4">
                <label className="text-sm font-bold text-slate-700">Key Amenities (Optional)</label>
                <div className="flex flex-wrap gap-3">
                  {['WiFi', 'Pool', 'Breakfast', 'Spa', 'Gym', 'Parking', 'Beach Access'].map(item => (
                    <button type="button" key={item} className="px-5 py-2 bg-slate-50 border border-slate-100 rounded-xl text-sm font-bold text-slate-600 hover:border-brand-500 hover:text-brand-600 transition-all">
                      {item}
                    </button>
                  ))}
                  <button type="button" className="flex items-center gap-2 px-5 py-2 bg-brand-50 border border-brand-100 rounded-xl text-sm font-bold text-brand-600">
