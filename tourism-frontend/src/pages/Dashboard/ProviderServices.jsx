    ...
    <div className="flex bg-slate-50 min-h-screen">
      ...
      <main className="flex-1 p-8 md:p-12 space-y-10 max-w-7xl mx-auto w-full">
        <header className="flex flex-col md:flex-row justify-between items-end gap-6 mb-8">
          <div>
            <h1 className="text-3xl font-bold tracking-tight font-heading">My Services</h1>
            <p className="text-slate-500 mt-2">Manage your listings, update details, and monitor performance.</p>
          </div>
          ...
          <div className="p-8 border-b border-slate-100 flex flex-col md:flex-row items-center justify-between gap-6">
            <div className="relative w-full md:w-96">
              ...
              <Input 
                ...
                className="w-full pl-12 h-12 rounded-2xl bg-slate-50 border-none focus:ring-2 focus:ring-brand-500"
              />
            </div>
            <div className="flex items-center gap-3">
               <span className="text-xs font-bold text-slate-400 uppercase tracking-widest bg-slate-50 px-4 py-2 rounded-xl border border-slate-100">Total: {filteredServices.length}</span>
            </div>
          </div>
                    ...
                    <tr key={service.id} className="hover:bg-slate-50/50 transition-colors group">
                      <td className="px-8 py-6">
                        <div className="flex items-center gap-5">
                          <div className="w-14 h-14 rounded-2xl overflow-hidden shadow-sm shrink-0 bg-slate-100">
                          ...
                          <div>
                            <h4 className="font-bold text-slate-900 text-base mb-1 font-heading">{service.name}</h4>
                      ...
                      <td className="px-8 py-6">
                        <div className="flex items-center gap-2 text-slate-600 bg-slate-100/50 px-3 py-1.5 rounded-lg w-max border border-slate-100">
                          <Tag className="w-3 h-3 text-brand-500" />
                          <span className="text-sm font-bold capitalize">{service.category}</span>
                        </div>
                      </td>
                      <td className="px-8 py-6">
                        <span className="text-lg font-bold text-slate-900 font-heading">${service.price}</span>
                      </td>
                      <td className="px-8 py-6">
                        <span className={cn(
                          "px-3 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-widest border",
                          service.is_available ? "bg-green-50 text-green-600 border-green-100" : "bg-red-50 text-red-500 border-red-100"
                        )}>
                      ...
                      <td className="px-8 py-6 text-right">
                        <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                          <Link to={`/services/${service.id}`}>
                            <Button variant="ghost" size="icon" className="h-10 w-10 text-slate-400 hover:text-brand-600 hover:bg-brand-50 rounded-xl">
                          ...
                          <Button variant="ghost" size="icon" onClick={() => handleDelete(service.id)} className="h-10 w-10 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-xl">
