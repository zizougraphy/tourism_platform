    ...
    <div className="flex bg-slate-50 dark:bg-slate-950 min-h-screen">
      ...
      <main className="flex-1 p-8 md:p-12 space-y-10 max-w-7xl mx-auto w-full">
        <header className="flex flex-col md:flex-row justify-between items-end gap-6 mb-8">
          <div>
            <h1 className="text-3xl font-bold tracking-tight font-heading dark:text-white">Booking Management</h1>
            <p className="text-slate-500 dark:text-slate-400 mt-2">Manage your incoming reservations and past bookings.</p>
          </div>
          <div className="flex gap-2 bg-white dark:bg-slate-900 p-1 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm">
            {['all', 'pending', 'confirmed', 'cancelled'].map(tab => (
              <button 
                key={tab} 
                onClick={() => setFilter(tab)}
                className={`px-6 py-2 rounded-xl text-sm font-bold capitalize transition-all ${
                  tab === filter ? 'bg-brand-600 text-white shadow-md' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-800'
                }`}
              >
                {tab}
              </button>
          ...
          <div className="overflow-x-auto">
            {loading ? (
              ...
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                  {filteredBookings.map((bk) => (
                    <tr key={bk.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors group">
                      <td className="px-8 py-6">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <h4 className="font-bold text-slate-900 dark:text-white text-sm mb-1 font-heading">{bk.tourist_name || 'Anonymous Guest'}</h4>
                      ...
                      <td className="px-8 py-6">
                        <div className="flex items-center gap-2 text-sm text-slate-700 dark:text-slate-300 font-medium bg-slate-100/50 dark:bg-slate-800/50 px-3 py-1.5 rounded-lg w-fit border border-slate-100 dark:border-slate-700">
                      ...
                      <td className="px-8 py-6">
                        <span className="text-lg font-bold text-slate-900 dark:text-white font-heading">${bk.total_price}</span>
                      </td>
                      <td className="px-8 py-6">
                        <span className={cn(
                          "px-3 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-widest border flex items-center gap-1.5 w-fit",
                          bk.status === 'confirmed' ? "bg-green-50 dark:bg-green-900/20 text-green-600 dark:text-green-400 border-green-100 dark:border-green-900/50" :
                          bk.status === 'pending' ? "bg-amber-50 dark:bg-amber-900/20 text-amber-600 dark:text-amber-400 border-amber-100 dark:border-amber-900/50" :
                      ...
                      <td className="px-8 py-6 text-right">
                        <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                          <Button onClick={() => navigate('/dashboard/messages', { state: { provider_id: bk.tourist_id, provider_name: bk.tourist_name || 'Guest' } })} variant="ghost" size="icon" className="h-10 w-10 text-brand-600 hover:bg-brand-50 dark:hover:bg-brand-900/20 rounded-xl">
              ...
              <div className="py-24 text-center">
                ...
                <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-2 font-heading">No Bookings Yet</h3>
