      ...
      <div className="max-w-5xl mx-auto space-y-12">
        <header className="flex flex-col md:flex-row justify-between items-end gap-6 border-b border-slate-200 pb-12">
          <div className="space-y-2">
            <h1 className="text-4xl md:text-5xl font-bold font-heading">Your Bookings</h1>
            ...
            {['All', 'Upcoming', 'Past'].map(tab => (
              <button 
                ...
                className={`px-6 py-2 rounded-xl text-sm font-bold transition-all ${
                  tab === activeTab ? 'bg-brand-600 text-white shadow-md' : 'text-slate-400 hover:text-slate-600 hover:bg-slate-50'
                }`}
              >
                {tab}
              </button>
        ...
        {loading ? (
          <div className="text-center py-24 text-slate-500 font-medium">Loading your bookings...</div>
          ...
          <div className="space-y-6">
            {filteredBookings.map((booking, i) => (
              ...
              >
                <Card className="p-0 border-none shadow-sm hover:shadow-xl transition-shadow duration-500 overflow-hidden flex flex-col md:flex-row bg-white rounded-[2rem]">
                  ...
                  <div className="flex-grow p-8 flex flex-col justify-between">
                    <div className="space-y-6">
                      <div className="flex justify-between items-start gap-4 flex-col sm:flex-row">
                        <div>
                          <h3 className="text-2xl font-bold mb-3 font-heading text-slate-900">{booking.service_name || 'Premium Service'}</h3>
                        ...
                        <div className="sm:text-right bg-brand-50/50 p-4 rounded-2xl border border-brand-100/50 shrink-0">
                          <div className="text-[10px] uppercase tracking-widest font-bold text-brand-600/70 mb-1">Total Amount</div>
                          <div className="text-2xl font-bold text-brand-900">${booking.total_price || booking.price || '0'}</div>
                        </div>
                      </div>
                      ...
                      <div className="flex flex-wrap gap-8 pt-4">
                        <div className="space-y-1">
                          ...
                          <div className="font-mono text-sm font-bold text-slate-700 bg-slate-100 px-2 py-1 rounded-md uppercase">#{booking.id.toString().slice(0,8)}</div>
                        </div>
                        <div className="space-y-1">
                          <div className="text-[10px] uppercase tracking-widest font-bold text-slate-400">Guests</div>
                          <div className="font-bold text-slate-700">{booking.guests || 1} Guests</div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="pt-6 mt-6 border-t border-slate-100 flex flex-col sm:flex-row justify-between items-center gap-4">
                      <div className="flex items-center gap-2 w-full sm:w-auto">
                        <Button onClick={() => navigate('/messages', { state: { provider_id: booking.provider_id, provider_name: booking.provider_name } })} variant="ghost" size="sm" className="font-bold text-brand-600 gap-2 bg-brand-50 hover:bg-brand-100 flex-1 sm:flex-none justify-center">
                        ...
                        <Link to={`/services/${booking.service_id}`} className="flex-1 sm:flex-none">
                          <Button variant="ghost" size="sm" className="font-bold text-slate-600 gap-2 bg-slate-50 hover:bg-slate-100 w-full justify-center">
                      ...
                      <div className="flex gap-2 w-full sm:w-auto">
                        {booking.status !== 'cancelled' && booking.status !== 'completed' && (
                          <Button 
                            variant="outline" 
                            size="sm" 
                            className="border-rose-200 text-rose-500 hover:bg-rose-50 hover:border-rose-300 flex-1 sm:flex-none"
          ...
          <div className="py-24 text-center bg-white rounded-[3rem] border border-slate-100 shadow-sm">
            ...
            <h3 className="text-2xl font-bold mb-3 font-heading">No bookings found</h3>
